...
| 按键 | 操作 |
| --- | --- |
| UP / DOWN 短按 | 在当页任务行间移动光标(左侧黄色光标条);**到顶/到底后循环**——第一条再按 UP 绕到最后一行,最后一行再按 DOWN 绕回第一条 |
| UP 长按 | 上一页(PREV) |
| DOWN 长按 | 下一页(NEXT) |
| OK 单击 | 勾选 / 取消当前任务,绿色实心;紧急任务勾选后转已完成 |
| OK 长按 | 删除确认(UP/DOWN 选择,OK 确认) |
| OK 双击 | 打开**任务详情页**:整屏自动换行显示完整主/副标题,单击或长按 OK 返回 |

翻页为机械式硬切换(无滑动动画),进度与页码只做局部刷新。

列表一行只有 176px:主标题约 12 个汉字、副标题约 14 个汉字。超出部分在列表里以 `…` 收尾;
**光标所在那行的副标题会自动来回滚动**显示全文(其余行保持 `…` 不动),
双击 OK 可在详情页看到完整的主/副标题。

列表最后一页还有 **「历史 / HISTORY」** 入口行(与「设置 / SETTINGS」并排):进入后列出所有已完成的
任务,最近完成的在前,上下键翻页、OK 返回。牌子上没有时钟,故只按**完成先后**排序、不显示时间;
带时间戳的完整历史在网页端看。空间不足时优先保留「设置」入口(该入口需要最后一页有 2 个空行)。
...
- 网页端带 **历史记录** 区:勾选完成会自动记入,按完成时间倒序,可搜索,可「恢复为未完成」,
  可「归档」(从牌子列表移走、记录留存)。时间戳来自服务端 `completedAt`(设备侧无时钟)。
  已完成项还支持 **一键归档**:批量把已完成的任务移出牌子列表、腾出名额(牌子最多 12 条)。

### 导入 JSON(批量)

网页端底部「**导入 JSON / IMPORT**」:选一个 `.json` → **预览**(显示会新增/更新多少条、导入后总数)→ **确认导入**。
带 `id` 的条目按 id 更新(同一文件重复导入不会产生重复任务),不带的自动生成 id。

```json
{
  "format": "eva-todo/import",   // 可省略
  "version": 1,                  // 可省略
  "mode": "merge",               // merge(默认:保留现有 + 追加/按 id 更新) | replace(整份替换)
  "tasks": [
...
- 顶层也可以直接是数组:`[ {...}, {...} ]`
- 字段:`title` 必填(≤79 字节) / `notes` 可选(≤149 字节) / `status`:`todo` 或 `done` /
  `urgent`(true 时牌子上变红) / `priority`(0-3,仅服务端记录,牌子不显示) / `id`(可选)
- 超长按字节安全截断(不会切断汉字);缺标题的条目会跳过并在结果里报告原因
- 导入后总条数超过设备上限 12 条会被拒绝(错误信息给出"现有 X + 文件 Y"与处理建议)
- 示例文件:`usb-web/sample-import.json`
