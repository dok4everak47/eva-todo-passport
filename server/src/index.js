...
function skillResponse(request) {
  ...
  return new Response(body, {
    headers: {
      ...
      "content-disposition": 'attachment; filename="todo-api-skill.md"',
      "cache-control": "no-store"
    }
  });
}

const SKILL_MD = `# Todo API Skill
...
.internal{border:2px solid var(--yellow);padding:8px 52px 8px 12px;position:relative;font-size:22px;font-weight:900}.internal small{display:block;font-size:13px}.slash{position:absolute;right:12px;top:8px;width:28px;height:12px;background:var(--red);transform:skew(-24deg)}.slash.s2{top:24px}.slash.s3{top:40px}
 .bar{border:2px solid var(--yellow);margin-top:12px;padding:12px;display:flex;gap:12px;align-items:center;flex-wrap:wrap}.bar input,.bar select,.bar button,.bar a,.card input,.card textarea,.card select{background:#050505;color:var(--yellow);border:1px solid var(--yellow);padding:8px;border-radius:0}.bar button,.bar a,.card button{cursor:pointer;font-weight:800}.bar a{text-decoration:none}.bar button:hover,.bar a:hover,.card button:hover{background:var(--yellow);color:#000}
...
<body><div class="wrap">
  ...
  <div class="bar">
     ...
     <button id="saveToken">SAVE TOKEN</button><button id="copySkill">COPY AI SKILL</button><a href="/skill.md" download>DOWNLOAD SKILL</a><span id="skillStatus" class="muted"></span>
